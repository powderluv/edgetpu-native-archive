#include "edgetpu/cpp/basic/basic_engine_native.h"
...
#include "edgetpu/cpp/basic/invalid_models/fake_op.h"
...
TEST(BasicEngineNativeBuilderTest, TestInvalidDevicePath) {
  BasicEngineNativeBuilder builder(
      ModelPath("mobilenet_v1_1.0_224_quant.tflite"), "/invalid/device/path");
  std::unique_ptr<BasicEngineNative> engine;
  EXPECT_EQ(kEdgeTpuApiError, builder(&engine));
...
TEST(BasicEngineNativeTest, TestNegativeTensorIndex) {
  ...
  BasicEngineNativeBuilder builder(
      ModelPath("mobilenet_v1_1.0_224_quant.tflite"));
  EXPECT_EQ(kEdgeTpuApiOk, builder(&engine));
  int tensor_size;
...
TEST(BasicEngineNativeTest, TestDebugFunctions) {
  ...
  EXPECT_EQ(ModelPath("mobilenet_v1_1.0_224_quant.tflite"), path);
}
...
TEST(BasicEngineNativeTest, TestRunInferenceFailure) {
  ...
  std::unique_ptr<tflite::FlatBufferModel> model(
      tflite::FlatBufferModel::BuildFromFile(
          ModelPath("../invalid_models/model_invoking_error.tflite").c_str()));
...
TEST(BasicEngineNativeTest, TestRunInferenceSuccess) {
  std::unique_ptr<BasicEngineNative> engine;
  BasicEngineNativeBuilder builder(
      ModelPath("mobilenet_v1_1.0_224_quant_edgetpu.tflite"));
  EXPECT_EQ(kEdgeTpuApiOk, builder(&engine));
  float const *result;
  int result_size;
