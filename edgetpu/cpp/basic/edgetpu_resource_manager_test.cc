#include "edgetpu/cpp/basic/edgetpu_resource_manager.h"
...
TEST_F(EdgeTpuResourceManagerTest, GetAllEdgeTpuOnce) {
  ...
  for (int i = 0; i < unassigned_devices_.size(); ++i) {
    ...
    EXPECT_EQ(
        i + 1,
        resource_manager_->ListEdgeTpuPaths(EdgeTpuState::kAssigned).size());
  }
}
...
TEST_F(EdgeTpuResourceManagerTest, CheckDevicePath) {
  ...
  CHECK_EQ(unassigned_devices_[0], edgetpu_resource->path());
}
