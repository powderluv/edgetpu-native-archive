...
DEFINE_string(posenet_data_dir,
              "knowledge/cerebra/spacepark/darwinn/git/edgetpu/cpp/"
              "posenet/test_data",
              "Posenet test data directory");

namespace coral {

std::string PosenetModelPath(const std::string& model_name) {
  if (model_name.empty()) return FLAGS_posenet_model_dir;
  return absl::StrCat(FLAGS_posenet_model_dir, "/", model_name);
}

std::string PosenetDataPath(const std::string& file_name) {
  if (file_name.empty()) return FLAGS_posenet_data_dir;
  return absl::StrCat(FLAGS_posenet_data_dir, "/", file_name);
}
...
void TestPoseNetDecoder(
  ...
  const std::vector<std::vector<float>>& raw_outputs = TestWithImage(
      PosenetModelPath(model_name), PosenetDataPath("test_image.bmp"));
