...
std::unique_ptr<tflite::ModelT> BuildTestGraph(
  ...
  AppendBuffer(/*buffer_size_bytes=*/0, model_t.get());
  ...
  auto input_tensor_quant =
      CreateQuantParam(/*min=*/{-128.0f}, /*max=*/{128.0f}, /*scale=*/{1.0f},
                       /*zero_point=*/{128});
  const int input_tensor_index = AppendTensor(
  ...
  const int conv2d_op_index = AppendOperator(
      tensor_configs, tflite::BuiltinOperator_CONV_2D, model_t.get());
  ...
  SetConv2dParams(quant_kernel, /*bias=*/{}, conv2d_op_index, model_t.get());
  return model_t;
}
...
TEST(UtilsTest, AppendL2Norm) {
  ...
  AppendL2Norm(model_t.get());
  EXPECT_EQ(model_t->subgraphs[0]->operators.size(), 2);

  const auto result = RunInference(
      model_t.get(), /*input_tensor=*/{1.0f, 2.0f, 3.0f, 4.0f, 5.0f});
  EXPECT_EQ(3, result.size());
  EXPECT_NEAR(1 / sqrt(11.0f), result[0], 0.01);
  EXPECT_NEAR(-1 / sqrt(11.0f), result[1], 0.01);
  EXPECT_NEAR(3 / sqrt(11.0f), result[2], 0.01);
}
...
TEST(UtilsTest, AppendFullyConnectedLayer) {
  ...
  auto model_t = BuildTestGraph(/*input_shape=*/{1, 1, 1, 5},
                                /*kernel_shape=*/{2, 1, 1, 5}, kernel);
  AppendL2Norm(model_t.get());
  const int fc_op_index =
      AppendFullyConnectedLayer(/*kernel_shape=*/{4, 1, 1, 2},
                                GenerateQuantParamsForConv2d(), model_t.get());
  // Set weights for fully-connected layer.
  const std::vector<float>& fc_weights = {
      sqrt(2.0f) / 2, -sqrt(2.0f) / 2,  // kernel 1
      sqrt(2.0f) / 2, sqrt(2.0f) / 2,   // kernel 2
      sqrt(7.0f) / 4, 3.0f / 4,         // kernel 3
      sqrt(5.0f) / 3, 2.0f / 3,         // kernel 4
  };
  ...
  SetConv2dParams(quant_fc_weights, /*bias=*/{}, fc_op_index, model_t.get());
  ...
  EXPECT_NEAR((sqrt(14.0f) - sqrt(18.0f)) / 8, result[2], 0.01);
  EXPECT_NEAR((sqrt(10.0f) - sqrt(8.0f)) / 6, result[3], 0.01);
}
...
TEST(UtilsTest, AppendReshape) {
  ...
  EXPECT_EQ(model_t->subgraphs[0]->operators.size(), 4);
  // Check graph's output tensor's shape.
  const std::vector<tflite::TensorT*> output_tensors =
      GetGraphOutputTensors(model_t.get());
  CHECK_EQ(output_tensors.size(), 1);
  const tflite::TensorT* current_output_tensor = output_tensors[0];
  EXPECT_EQ(current_output_tensor->shape.size(), 2);
...
TEST(UtilsTest, AppendSoftmax) {
  ...
  std::vector<float> unnormalized_results = {
      exp(1.0f), exp(0.0f), exp((sqrt(14.0f) - sqrt(18.0f)) / 8),
      exp((sqrt(10.0f) - sqrt(8.0f)) / 6)};
...
// Test appending a fake custom operator. It only checks model structure.
TEST(UtilsTest, AppendCustomOp) {
  // The testee model has two operators, conv and l2norm.
  const std::vector<float> kernel = {
      1.0f,  1.0f,  1.0f,  1.0f,  1.0f,   // kernel 1
      -1.0f, -1.0f, -1.0f, -1.0f, -1.0f,  // kernel 2
  };
  auto model_t = BuildTestGraph(/*input_shape=*/{1, 1, 1, 5},
                                /*kernel_shape=*/{2, 1, 1, 5}, kernel);
  AppendL2Norm(model_t.get());
  ASSERT_EQ(2, model_t->operator_codes.size());
  ASSERT_EQ(1, model_t->subgraphs[0]->inputs.size());
  ASSERT_EQ(1, model_t->subgraphs[0]->outputs.size());
  ASSERT_EQ(2, model_t->subgraphs[0]->operators.size());
  ASSERT_EQ(5, model_t->subgraphs[0]->tensors.size());

  // Insert a custom op named "FakeOp" after conv.
  auto conv2d_output_quant =
      CreateQuantParam(/*min=*/{-128.0f}, /*max=*/{128.0f}, /*scale=*/{1.0f},
                       /*zero_point=*/{128});
  auto conv2d_kernel_quant =
  ...
  std::vector<TensorConfig> tensor_configs;
  // For existing tensors only name matters.
  tensor_configs.push_back({"TestGraph/Conv2d/Output",
                            tflite::TensorType_UINT8,
                            TensorLocation::kInput,
                            {},
                            nullptr});
  tensor_configs.push_back({"TestGraph/Conv2d/Kernel",
                            tflite::TensorType_UINT8,
                            TensorLocation::kParameter,
                            {},
                            nullptr});
  // New tensors must have fully specified configuration.
  tensor_configs.push_back({"TestGraph/FakeOp/Kernel",
                            tflite::TensorType_FLOAT32,
                            TensorLocation::kParameter,
                            {5, 4, 3, 2},
                            nullptr});
  tensor_configs.push_back({"TestGraph/FakeOp/Output",
                            tflite::TensorType_FLOAT32,
  ...
  fbb.Map([&fbb]() {
    fbb.Int("fake_param0", 7);
    fbb.Float("fake_param1", 0.3);
  });
  fbb.Finish();
  const int fake_op_index =
      AppendOperator(tensor_configs, tflite::BuiltinOperator_CUSTOM, "FakeOp",
                     fbb.GetBuffer(), model_t.get());

  EXPECT_EQ(2, fake_op_index);
  EXPECT_EQ(3, model_t->operator_codes.size());
  EXPECT_EQ(1, model_t->subgraphs[0]->inputs.size());
  EXPECT_EQ(1, model_t->subgraphs[0]->outputs.size());
  EXPECT_EQ(3, model_t->subgraphs[0]->operators.size());
  EXPECT_EQ(7, model_t->subgraphs[0]->tensors.size());
}

// Test detaching operator. It only checks model structure.
TEST(UtilsTest, DetachTrailingOperator) {
  // The testee model has two operators, conv and l2norm.
  const std::vector<float> kernel = {
      1.0f,  1.0f,  1.0f,  1.0f,  1.0f,   // kernel 1
      -1.0f, -1.0f, -1.0f, -1.0f, -1.0f,  // kernel 2
  ...
  // No need to provide quant parameters for this operation.
  tflite::OperatorT deleted_op;
  ASSERT_TRUE(DetachTrailingOperator(tflite::BuiltinOperator_L2_NORMALIZATION,
                                     "", model_t.get(), &deleted_op));

  EXPECT_EQ(2, model_t->operator_codes.size());
  EXPECT_EQ(1, model_t->subgraphs[0]->inputs.size());
  EXPECT_EQ(1, model_t->subgraphs[0]->outputs.size());
  EXPECT_EQ(1, model_t->subgraphs[0]->operators.size());
  EXPECT_EQ(5, model_t->subgraphs[0]->tensors.size());
  EXPECT_EQ(1, deleted_op.opcode_index);
}
...
TEST(UtilsTest, FindSingleOperator) {
  const std::string& model_path =
      ModelPath("mobilenet_v1_1.0_224_quant.tflite");
  const auto model_t = LoadModel(model_path);
  const tflite::BuiltinOperator target_op = tflite::BuiltinOperator_SOFTMAX;
  const int softmax_op_index = FindSingleOperator(target_op, model_t.get());
...
TEST(UtilsTest, AppendFullyConnectedAndSoftmaxLayerToModel) {
  const std::string& in_model_path = ModelPath(
...
TEST(UtilsTest,
  ...
  const std::string& in_model_path = ModelPath("invalid_path.tflite");
  const std::string out_model_path = "/tmp/retrained_model_edgetpu.tflite";
