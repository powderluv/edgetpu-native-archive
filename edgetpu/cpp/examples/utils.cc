...
std::vector<uint8_t> ReadBmp(const std::string& input_bmp_name,
  ...
  const int32_t header_size = to_int32(img_bytes.data() + 10);
  const int32_t bpp = to_int32(img_bytes.data() + 28);
  int* width = image_dims->data() + 1;
  int* height = image_dims->data();
  int* channels = image_dims->data() + 2;
  *width = to_int32(img_bytes.data() + 18);
  *height = to_int32(img_bytes.data() + 22);
  *channels = bpp / 8;
